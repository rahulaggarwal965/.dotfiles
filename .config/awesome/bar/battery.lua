...
awesome.connect_signal("signal::battery", function(value, state)
	for _, instance in pairs(M.instances) do
		...
		if value <= 20 then
			instance.progress.color = "#F44336" -- Red when low
			instance.cap.bg = "#F44336"
		else
			-- Normal foreground color for everything else
			instance.progress.color = beautiful.fg_normal or "#cccccc"
			instance.cap.bg = beautiful.fg_normal or "#cccccc"
		end
	end
end)
