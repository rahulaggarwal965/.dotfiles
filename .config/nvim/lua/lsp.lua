...
M.on_attach = function(client)
    ...
    vim.keymap.set(
        ...
        { buffer = true, desc = "LSP: Previous Error" }
    )
    ...
    vim.keymap.set(
        ...
        "]e",
        function() vim.diagnostic.jump({ count = 1, severity = vim.diagnostic.severity.ERROR }) end,
        { buffer = true, desc = "LSP: Next Error" }
    )
    ...
    vim.keymap.set(
        ...
        { buffer = true, desc = "Search: project diagnostics" }
    )
    ...
    if client.server_capabilities.document_formatting then
        vim.keymap.set(
            ...
            { buffer = true, desc = "LSP: Format document" }
        )
    end
end
...
local meta = {
    __index = function(_, server_name)
        ...
        for _, configured_server_name in pairs(M.configured_servers) do
            if server_name == configured_server_name then return { setup = function() end } end
        end
        ...
        return {
            setup = function(config)
                ...
                if config.on_attach == nil then config.on_attach = M.on_attach end
