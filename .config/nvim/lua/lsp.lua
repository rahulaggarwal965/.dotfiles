...
M.on_attach = function(client)
	...
	vim.keymap.set(
		...
		{ buffer = true, desc = "Search: references for symbol" }
	)
	vim.keymap.set(
		"n",
		"<leader>ld",
		telescope.lsp_definitions,
		{ buffer = true, desc = "Search: definitions for symbol" }
	)
	vim.keymap.set(
		"n",
		"<leader>li",
		telescope.lsp_implementations,
		{ buffer = true, desc = "Search: implementations for symbols" }
	)
	...
	vim.keymap.set(
		...
		{ buffer = true, desc = "Search: project symbols" }
	)
	vim.keymap.set("n", "<leader>lg", function()
		telescope.diagnostics({ bufnr = 0 })
	end, { buffer = true, desc = "Search: project diagnostics" })
	vim.keymap.set("n", "<leader>lG", telescope.diagnostics, { buffer = true, desc = "Search: project diagnostics" })

	if client.server_capabilities.document_formatting then
		vim.keymap.set("n", "<leader>lf", function()
			vim.lsp.buf.format({ async = true })
		end, { buffer = true, desc = "LSP: Format document" })
	end
end
