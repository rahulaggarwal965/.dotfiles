			...
			return {
				...
				on_attach = function(bufnr)
					...
					vim.keymap.set(
						...
						{ buffer = bufnr, desc = "File Explorer: cd down" }
					)
					vim.keymap.set(
						"n",
						"H",
						api.tree.change_root_to_parent,
						{ buffer = bufnr, desc = "File Explorer: cd up" }
					)
					vim.keymap.set(
						"n",
						"<C-s>",
					...
					vim.keymap.set(
						...
						{ buffer = bufnr, desc = "File Explorer: Close node" }
					)
					vim.keymap.set(
						"n",
						".",
						api.tree.toggle_hidden_filter,
						{ buffer = bufnr, desc = "File Explorer: Toggle hidden files" }
					)
				end,
				renderer = {
					highlight_git = true,
